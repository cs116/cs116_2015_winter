...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=   
{
   ...
   rmCmd = "/bin/sed 's/\\x1//g' " + fn + " > " + fn2;
   system(rmCmd.c_str());
   rmCmd = "/bin/cp " + fn2 + " " + fn;
   system(rmCmd.c_str());
   rmCmd = "/bin/sed 's/\\x0//g' " + fn + " > " + fn2;
   system(rmCmd.c_str());
   rmCmd = "/bin/cp " + fn2 + " " + fn;
   system(rmCmd.c_str());
...
DirType* SubType::SortList(DirType*& list)
{
   DirType* tmpList1;
   tmpList1 = new DirType;
   DirType* tmpList2;
   tmpList2 = new DirType;

   string tmpfn;

   for(tmpList1 = list; tmpList1 != NULL; tmpList1 = tmpList1->next)
   {
      for(tmpList2 = tmpList1->next; tmpList2 != NULL; tmpList2 = tmpList2->next)
      {
         if(tmpList1->info > tmpList2->info)
         {
            tmpfn = tmpList1-> info;
            tmpList1->info = tmpList2->info;
            tmpList2->info = tmpfn;
         }
      }
   }
